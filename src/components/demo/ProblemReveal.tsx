...
export const ProblemReveal = ({ amplitude, bass, isPlaying }: ProblemRevealProps) => {
  return (
    ...
    >
      ...
      <div className="absolute -inset-8 -z-10 rounded-3xl overflow-hidden">
        <img
          src={dawInterfaceHero}
          alt=""
        ...
        <div 
          ...
          style={{
            background: `linear-gradient(to bottom, hsl(var(--background) / 0.8) 0%, hsl(var(--background) / 0.4) 40%, hsl(var(--background) / 0.4) 60%, hsl(var(--background) / 0.8) 100%)`
          }}
        />
      </div>
      ...
      >
        ...
        >
          <motion.div
            className="w-16 h-16 rounded-2xl bg-destructive/20 border border-destructive/40 flex items-center justify-center backdrop-blur-sm"
            animate={{
              boxShadow: isPlaying
      ...
      {/* Problem Stats Grid — Glassmorphic Container */}
      <motion.div
        className="rounded-2xl bg-background/60 backdrop-blur-md border border-border/20 p-6"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {problems.map((problem, index) => {
            ...
            return (
              <motion.div
                key={index}
                className="p-4 rounded-xl bg-destructive/5 border border-destructive/20 backdrop-blur-sm"
                initial={{ opacity: 0, scale: 0.9 }}
                animate={{ opacity: 1, scale: 1 }}

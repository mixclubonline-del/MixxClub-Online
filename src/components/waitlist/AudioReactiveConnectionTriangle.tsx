...
interface AudioReactiveConnectionTriangleProps {
  bass?: number;
  mid?: number;
  high?: number;
  amplitude?: number;
...
export function AudioReactiveConnectionTriangle({ 
  bass = 0, 
  mid = 0, 
  high = 0,
  amplitude = 0,
  isPlaying = false 
}: AudioReactiveConnectionTriangleProps) {
  // Audio-reactive values
  const bassNorm = bass / 255;
  const midNorm = mid / 255;
  const highNorm = high / 255;
  const ampNorm = amplitude / 255;

  // Line stroke width reacts to amplitude
  const strokeWidth = isPlaying ? 0.3 + ampNorm * 0.5 : 0.3;
  
  // Glow intensity reacts to bass
  const glowScale = isPlaying ? 1 + bassNorm * 0.4 : 1;
  const glowOpacity = isPlaying ? 0.3 + bassNorm * 0.5 : 0.3;

  // Node scale reacts to mid
  const nodeScale = isPlaying ? 1 + midNorm * 0.1 : 1;

  // Energy pulse speed reacts to high
  const pulseSpeed = isPlaying ? 2 - highNorm * 0.5 : 2;
    ...
    <div className="relative w-full max-w-2xl mx-auto aspect-square">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
        ...
        <motion.line
          ...
          animate={{ pathLength: 1, opacity: 0.6 + bassNorm * 0.3 }}
        ...
        {/* Energy pulse on lines - speed reacts to audio */}
        <motion.circle
          r={isPlaying ? 1 + bassNorm * 0.5 : 1}
      ...
      {/* Center text with audio-reactive pulse */}
      <motion.div
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center"
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ 
          opacity: 1, 
          scale: isPlaying ? 1 + ampNorm * 0.05 : 1 
        }}
        transition={{ delay: 2, duration: 0.8 }}
      >
        <p className="text-sm uppercase tracking-[0.3em] text-muted-foreground">The</p>
        <motion.h2 
          className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-primary via-accent-magenta to-accent-blue bg-clip-text text-transparent"
          animate={{
            textShadow: isPlaying && bass > 150 
              ? `0 0 ${20 + bassNorm * 20}px hsl(var(--primary) / 0.5)`
              : 'none'
          }}
        >
          Connection
      ...
      {/* Triangle Nodes with audio reactivity */}
      {nodes.map((node, index) => (
        ...
        >
          ...
          <motion.div
            ...
            transition={{ 
              duration: isPlaying ? 0.5 + (1 - bassNorm) * 2 : 3, 
              repeat: Infinity, 
              delay: index * 0.5 
            }}
          />

/**
 * CollectiveAnalytics — Migrated to GlassPanel/HubSkeleton design tokens.
 */

import { useMemo } from 'react';
import { DollarSign, TrendingUp, TrendingDown, Users, Star, BarChart3 } from 'lucide-react';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { useAuth } from '@/hooks/useAuth';
import { GlassPanel, HubSkeleton } from '../design';
...
function useCollectiveAnalytics() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CollectiveData> => {
      ...
      const partnershipProjects = projects.filter((p) => p.user_id && p.engineer_id);
      const soloProjects = projects.filter((p) => !p.engineer_id);
      ...
      projects.forEach((p) => {
        ...
        if (!entry) return;
        if (p.user_id && p.engineer_id) entry.partnership += 1;
        else entry.solo += 1;
      });
...
function ComparisonChart({ data }: { data: PartnershipRevenue[] }) {
  ...
  return (
    <GlassPanel padding="p-6">
      <h3 className="font-semibold mb-1 text-foreground">Solo vs Partnership Revenue</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Partnership Revenue', value: `$${data.totalPartnershipRevenue.toLocaleString()}` },
          { label: 'Solo Revenue', value: `$${data.totalSoloRevenue.toLocaleString()}` },
          { label: 'Active Partners', value: data.activePartners, icon: <Users className="w-5 h-5 text-primary" /> },
          { label: 'Monthly Growth', value: `${data.partnershipGrowthRate}%`, icon: <GrowthIcon className="w-5 h-5" />, color: data.partnershipGrowthRate >= 0 ? 'text-green-500' : 'text-red-500' },
        ].map((stat) => (
          <GlassPanel key={stat.label} padding="p-5" hoverable>
            <p className="text-sm text-muted-foreground">{stat.label}</p>
            <p className={`text-2xl font-bold mt-1 flex items-center gap-2 ${stat.color || 'text-foreground'}`}>
              {stat.icon}
              {stat.value}
            </p>
          </GlassPanel>
      ...
      {liftPercent !== null && (
        <GlassPanel
          padding="p-5"
          accent={liftPercent > 0 ? 'rgba(34, 197, 94, 0.3)' : 'rgba(249, 115, 22, 0.3)'}
          className={liftPercent > 0 ? 'border-green-500/30' : 'border-orange-500/30'}
        >

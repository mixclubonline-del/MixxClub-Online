...
export default function QuickStart() {
  ...
  return (
    ...
    <div className="min-h-[100svh] relative flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="fixed inset-0 opacity-[0.03]" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=\'0 0 256 256\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cfilter id=\'n\'%3E%3CfeTurbulence type=\'fractalNoise\' baseFrequency=\'0.9\' numOctaves=\'4\' stitchTiles=\'stitch\'/%3E%3C/filter%3E%3Crect width=\'100%25\' height=\'100%25\' filter=\'url(%23n)\'/%3E%3C/svg%3E")' }} />

      {/* Glow orbs */}
      <div className="fixed top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full bg-primary/8 blur-[120px] animate-pulse" />
      <div className="fixed bottom-[-20%] right-[-10%] w-[400px] h-[400px] rounded-full bg-accent/6 blur-[100px] animate-pulse [animation-delay:2s]" />

      {/* Glassmorphic card */}
      <div className="relative z-10 w-full max-w-lg bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-8 space-y-6">
          ...
          {[0, 1, 2].map((i) => (
            <div
              ...
              className={`h-1 rounded-full transition-all duration-500 ${
                i <= stepIndex
                  ? 'w-12 bg-gradient-to-r from-primary to-primary-glow shadow-[0_0_8px_hsl(var(--primary)/0.5)]'
                  : 'w-8 bg-white/10'
              }`}
            />
          ))}
          ...
          <div className="space-y-5 animate-in fade-in duration-300">
            <div className="text-center space-y-1">
              <h1 className="text-2xl font-bold bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent">
                What brings you to Mixxclub?
              </h1>
              <p className="text-white/50 text-sm">Pick your role — you can always change it later.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {ROLES.map(({ id, label, tagline, icon: Icon }) => (
                <button
                  ...
                  className="flex flex-col items-center gap-2 rounded-xl p-5 bg-white/5 border border-white/10
                    hover:border-primary/60 hover:bg-primary/10 hover:shadow-[0_0_20px_hsl(var(--primary)/0.2)]
          ...
          <div className="space-y-5 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                ...
                disabled={loading}
                className="h-11 bg-white/5 border-white/10 hover:border-primary/40"
              >
            ...
            <form onSubmit={handleEmailAuth} className="space-y-3">
              <Input
                ...
                required
                autoComplete="email"
                className="h-11 bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-primary/50"
              />
              <div className="relative">
                <Input
                  ...
                  className="h-11 pr-10 bg-white/5 border-white/10 text-white placeholder:text-white/30 focus:border-primary/50"
                />
                <button
                  type="button"
        ...
        {step === 'action' && (
          <div className="space-y-5 animate-in fade-in duration-300">
            <div className="text-center space-y-1">
              <h1 className="text-xl font-bold bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent">
                You're in. What's first?
              </h1>
              <p className="text-white/50 text-sm">Pick your first move — you can explore everything later.</p>
            </div>
              ...
              {ACTIONS.map((action) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-4 rounded-xl p-4 bg-white/5 border border-white/10
                      hover:border-primary/60 hover:bg-primary/10 hover:shadow-[0_0_20px_hsl(var(--primary)/0.2)]
                  ...
                  >
                    ...
                    <div>
                      <span className="font-semibold text-sm text-white/90">{action.label}</span>
                      <p className="text-xs text-white/40">{action.desc}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 ml-auto text-white/30 group-hover:text-primary transition-colors" />
                  </button>
                );
              })}
            </div>

...
export default function ProducerProfile() {
  ...
  const { data: beats } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('producer_beats')
        .select('id, title, bpm, key_signature, genre, price_cents, license_type, preview_url, audio_url, status, created_at, producer_id, cover_image_url, downloads, exclusive_price_cents')

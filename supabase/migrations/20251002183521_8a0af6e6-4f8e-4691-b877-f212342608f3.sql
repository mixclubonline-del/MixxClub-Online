-- Insert marketplace items using the first available user as seller
DO $$
DECLARE
  mock_seller_id uuid;
  hip_hop_cat_id uuid;
  ...
  sfx_cat_id uuid;
BEGIN
  -- Get the first user from profiles to use as mock seller
  SELECT id INTO mock_seller_id FROM auth.users LIMIT 1;
  
  -- If no users exist, exit
  IF mock_seller_id IS NULL THEN
    RAISE NOTICE 'No users found, skipping marketplace items insertion';
    RETURN;
  END IF;
  ...
  -- Hip Hop & Trap Sample Packs
  (
    mock_seller_id, hip_hop_cat_id,
    '808 King Bundle',
  ...
  -- Electronic & EDM Sample Packs
  (
    mock_seller_id, edm_cat_id,
  ...
  -- Vocals & Instruments Sample Packs
  (
    mock_seller_id, vocal_cat_id,
  ...
  -- Sound Effects Sample Packs
  (
    mock_seller_id, sfx_cat_id,
  ...
  RAISE NOTICE 'Successfully inserted 15 marketplace items';
END $$;